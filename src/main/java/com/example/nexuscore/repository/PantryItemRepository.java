...
public interface PantryItemRepository extends JpaRepository<PantryItem, Integer> {
    ...
    @Query("select coalesce(sum(p.quantity), 0) from PantryItem p where p.profile.id = :profileId and p.food.id = :foodId")
    Integer sumQuantityByProfileIdAndFoodId(@Param("profileId") Integer profileId, @Param("foodId") Integer foodId);
}
